...
# КаменьНожницыБумага (кинуть вызов)
    @commands.command()
    async def roll_start(self,ctx,message, member: discord.Member = None, take = None, rps = ['камень', 'ножницы', 'бумагу']):
        if ctx.channel.id == cursor.execute("SELECT rps_id FROM config WHERE server_id = {}".format(ctx.guild.id)).fetchone()[0]:   
            if member is None:
            ...
            await ctx.send("Эта команда не работает в этом чате")
# КаменьНожницыБумага (принять вызов)
    ...
    async def rollend(self,ctx, member: discord.Member = None, take = None, rps = ['камень', 'ножницы', 'бумагу']):
        if ctx.channel.id == cursor.execute("SELECT rps_id FROM config WHERE server_id = {}".format(ctx.guild.id)).fetchone()[0]:       
            ...
            await ctx.send("Эта команда не работате в этом чате")
