'''
Python bot for Discord
last update: 15 may 2020

by Nikita [thruwol] Yarosh
'''
import config
import discord
import os
from discord import utils
from discord.ext import commands
from discord.utils import get

# events. Добавление ролей по реакциям
class MyClient(discord.Client):

# Выводит информацию об успешном подключении
    async def on_ready(self):
        print('Бот успешно подключился к серверу'.format(self.user))

# Вызывается когда добавляется реакция
    async def on_raw_reaction_add(self, payload):
        if payload.message_id == config.POST_ID:
            channel = self.get_channel(payload.channel_id) # получаем объект канала
            message = await channel.fetch_message(payload.message_id) # получаем объект сообщения
            member = utils.get(message.guild.members, id=payload.user_id) # получаем объект пользователя который поставил реакцию

            try:
                emoji = str(payload.emoji) # эмоджик который выбрал юзер
                role = utils.get(message.guild.roles, id=config.ROLES[emoji]) # объект выбранной роли (если есть)

                if(len([i for i in member.roles if i.id not in config.EXCROLES]) <= config.MAX_ROLES_PER_USER):
                    await member.add_roles(role)
                    print('[SUCCESS] Пользователь {0.display_name} получил роль {1.name}'.format(member, role))
                else:
                    await message.remove_reaction(payload.emoji, member)
                    print('[ERROR] Слишком много ролей у пользователя {0.display_name}'.format(member))

            except KeyError as e:
                print('[ERROR] Такой роли не найдено ' + emoji)
            except Exception as e:
                print(repr(e))

# Вызывается когда удаляется реакция
    async def on_raw_reaction_remove(self, payload):
        channel = self.get_channel(payload.channel_id) # получаем объект канала
        message = await channel.fetch_message(payload.message_id) # получаем объект сообщения
        member = utils.get(message.guild.members, id=payload.user_id) # получаем объект пользователя который поставил реакцию

        try:
            emoji = str(payload.emoji) # эмоджик который выбрал юзер
            role = utils.get(message.guild.roles, id=config.ROLES[emoji]) # объект выбранной роли (если есть)

            await member.remove_roles(role)
            print('[SUCCESS] Пользователь {1.name} убрал роль {0.display_name}'.format(member, role))

        except KeyError as e:
            print('[ERROR] Такой роли не найдено ' + emoji)
        except Exception as e:
            print(repr(e))

client = MyClient(command_prefix='.')
client.run(os.environ.get('BOT_TOKEN'))
